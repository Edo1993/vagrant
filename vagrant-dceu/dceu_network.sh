...
#./etcd --advertise-client-urls http://10.0.40.10:4001 --listen-client-urls http://0.0.0.0:4001
start_kv_consul() {
...
start_kv_etcd() {
  ...
  ./etcd --advertise-client-urls http://10.0.40.10:4001 --listen-client-urls http://0.0.0.0:4001 > etcd.log &
}
...
calico_setup() {
  ...
  export ETCD_AUTHORITY="10.0.40.10:4001"
...
main() {
  set -x
  commands=("start_kv_consul" "start_kv_etcd" "docker_conf_consul" "docker_conf_etcd" "calico_setup" "testenv")
  select cmd_to_execute in "${commands[@]}"; do
    case $cmd_to_execute in
      "start_kv_consul") start_kv_consul;;
      "start_kv_etcd") start_kv_etcd;;
